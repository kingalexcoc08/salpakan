...
export function createSessionRouter(sessionStore: SessionStore, matchStore: MatchStore): Router {
  ...
  router.post("/", (req, res) => {
    ...
    const { session, tokens } = sessionStore.createSession({
      captureMode,
      flagVsFlagRule,
      creatorColor,
      blueName,
  ...
  router.post("/join", (req, res) => {
    ...
    try {
      const { session, color, token } = sessionStore.joinSession(code.toUpperCase(), name);
      res.status(200).json({
        sessionId: session.id,
        code: session.code,
        captureMode: session.capture_mode,
      ...
      res.status(statusByError[message] ?? 400).json({ error: message });
    }
  });

  router.get("/:sessionId", (req, res) => {
    const session = sessionStore.getSessionById(req.params.sessionId);
    if (!session) {
    ...
    res.json({
      ...
      challengeCount: matchStore.listChallenges(session.id).length,
    });
  });

  // Manual end-of-game flag (spec §4.4 — full board/win-condition tracking is out of scope for MVP).
  router.post("/:sessionId/end", requireAuth(sessionStore), async (req, res) => {
    ...
    const openChallenge = matchStore.getOpenChallenge(session.id);
    if (openChallenge) {
      res.status(409).json({
    ...
    sessionStore.markEnded(session.id);
    ...
    matchStore.clearPhotoPaths(session.id);
